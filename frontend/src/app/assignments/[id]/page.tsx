...
import { QuizRunner, type QuizResult } from "@/components/assignments/QuizRunner";
import { QuizReview } from "@/components/assignments/QuizReview";
...
export default function AssignmentDetailPage() {
  ...
  useEffect(() => {
    api
      ...
      .then((data) => {
        const found = data.assignments.find((a) => a.id === params.id);
  ...
  async function handleComplete(quiz?: QuizResult) {
    ...
    try {
      ...
      if (result.pendingApproval) {
        ...
        toast.success(`+${assignment.xpReward} XP, +${quiz.correctCount * 5} coins`, {
          description: `${quiz.correctCount} / ${quiz.answers.length} correct.`,
        });
      } else {
        toast.success(`+${assignment.xpReward} XP, +${assignment.coinReward} coins`, {
      ...
      setTimeout(() => router.push("/assignments"), 1200);
    } catch (err) {
      setError(err instanceof ApiRequestError ? err.message : "Could not complete this assignment.");
      setCompleting(false);
    }
  }
  ...
  const hasQuiz = Boolean(assignment.quiz && assignment.quiz.length > 0);
    ...
    <PageIn className={hasQuiz ? "w-full" : "max-w-2xl"}>
      ...
      <Card className="glow-primary bg-grid relative overflow-hidden border-0">
        ...
        <CardContent className="relative px-6 py-6">
          ...
          <h1 className="font-display text-2xl tracking-wide uppercase">{assignment.title}</h1>
          <p className="mt-2 leading-relaxed text-muted-foreground">{assignment.description}</p>
          ...
          <div className="mt-5 flex gap-4 text-sm">
            <span className="flex items-center gap-1.5">
              ...
              <span className="tabular font-medium">{assignment.xpReward}</span>
              <span className="text-muted-foreground">XP</span>
            </span>
            <span className="flex items-center gap-1.5">
              <Coins className="size-4 text-currency" />
              <span className="tabular font-medium">
                {assignment.quiz && assignment.quiz.length > 0 ? `up to ${assignment.coinReward}` : assignment.coinReward}
              </span>
              <span className="text-muted-foreground">coins</span>
            </span>
          </div>

          {isActiveQuiz && assignment.quiz && (
            <>
              {error && <p className="mt-3 text-sm text-destructive">{error}</p>}
              <QuizRunner
                questions={assignment.quiz}
          ...
          {isCompletedQuiz && assignment.quiz && progress?.quizAnswers && (
            <QuizReview questions={assignment.quiz} answers={progress.quizAnswers} />
          )}
