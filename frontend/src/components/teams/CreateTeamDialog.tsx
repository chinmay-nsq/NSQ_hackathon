...
/** Manager/admin-only: creates a new team (team). The creating manager becomes its lead. */
export function CreateTeamDialog({ onCreated }: { onCreated: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.post("/teams", { name: name.trim(), department: department.trim() });
      setOpen(false);
      reset();
      onCreated();
    } catch (err) {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mt-5 space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="team-name" className="font-mono text-xs tracking-wide uppercase">
                Team name
              </Label>
              <Input
                id="team-name"
                ...
                placeholder="e.g. Nightwatch Team"
              />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="team-department" className="font-mono text-xs tracking-wide uppercase">
                Department
              </Label>
              <Input
                id="team-department"
