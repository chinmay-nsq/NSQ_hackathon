...
export function TaskDetailDialog({
  ...
  function load() {
    if (!taskId) return;
    api
      .get<TaskDetail>(`/adventures/${taskId}/detail`)
      .then((data) => {
        setDetail(data);
  ...
  async function handleAddComment() {
    ...
    try {
      await api.post(`/adventures/${taskId}/comments`, { body: commentBody.trim() });
      setCommentBody("");
      load();
    } catch (err) {
  ...
  async function handleMarkComplete() {
    ...
    setActing(true);
    try {
      await api.post(`/adventures/${taskId}/complete`, { submission: submission.trim() || undefined });
  ...
  async function handleApprove() {
    ...
    setActing(true);
    try {
      await api.post(`/adventures/${taskId}/approve/${detail.adventure.assignee.id}`);
  ...
  async function handleMoveSprint(nextSprintId: string) {
    ...
    try {
      await api.post(`/adventures/${taskId}/sprint`, { sprintId: nextSprintId || null });
  ...
  const adventure = detail?.adventure;
  const workItem = WORK_ITEM_STYLE[adventure?.workItemType ?? "TASK"];
  const column = COLUMN_LABEL[adventure?.column ?? "todo"];
  const isMyTask = adventure?.assignee?.id === employee?.id;
        ...
        {loading ? (
          ...
          <>
            <DialogHeader>
              ...
              <DialogTitle className="font-display text-xl tracking-wide">{adventure.title}</DialogTitle>
            </DialogHeader>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm">
              ...
              <span className="flex items-center gap-1.5">
                ...
                {adventure.coinReward} coins
              </span>
              {adventure.assignee && (
                <span className="flex items-center gap-1.5 text-muted-foreground">
                  <User className="size-4" />
                  Assigned to {adventure.assignee.name}
                </span>
              )}
            </div>
            ...
            {adventure.column === "todo" && isMyTask && (
