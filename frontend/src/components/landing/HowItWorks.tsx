    ...
    <section ref={scope} className="relative flex min-h-screen flex-col justify-center px-6 py-24 sm:px-12">
      <div className="mx-auto w-full max-w-7xl">
        <div className="how-head mb-12 max-w-2xl">
          <p className="font-mono text-xs uppercase tracking-[0.25em] text-(--smoke)">{copy.eyebrow}</p>
          <h2 className="mt-3 font-display text-[clamp(2rem,6vw,4.2rem)] leading-[0.95] text-(--ink)">
            {copy.heading}
          </h2>
          <p className="mt-5 max-w-lg text-base leading-relaxed text-(--smoke)">{copy.sub}</p>
        </div>

        <div className="how-head mb-10 inline-flex w-fit items-center gap-1 rounded-full border border-(--line) bg-(--wash) p-1">
          {(["employee", "lead"] as const).map((p) => (
            <button
              ...
              className={cn(
                ...
                persona === p ? "bg-brand text-(--paper)" : "text-(--smoke) hover:text-(--ash)"
              )}
            >
        ...
        <div className="relative grid gap-px overflow-hidden rounded-3xl border border-(--line) bg-(--wash) sm:grid-cols-2 lg:grid-cols-3">
          {steps.map((step, i) => (
            <div key={step.title} className="how-step relative flex flex-col gap-4 bg-(--paper) p-8">
              <div className="flex items-center justify-between">
                <step.icon className="size-7 text-brand" strokeWidth={1.5} />
              ...
              <div>
                ...
                <p className="mt-3 max-w-sm text-sm leading-relaxed text-(--smoke)">{step.body}</p>
              </div>
              {i < steps.length - 1 && (
                <span className="pointer-events-none absolute -right-px top-1/2 hidden h-px w-6 -translate-y-1/2 bg-linear-to-r from-(--brand) to-transparent sm:block lg:-right-px" />
              )}
            </div>
          ))}
        </div>
