  ...
  return (
    <section ref={scope} className="relative flex min-h-screen flex-col items-center justify-center px-6 sm:px-12">
      <div className="spotlight-content flex w-full max-w-3xl flex-col items-center gap-10 text-center">
        <div>
          <p className="font-mono text-xs uppercase tracking-[0.25em] text-(--smoke)">The Companion</p>
          <h2 className="mt-3 max-w-2xl font-display text-[clamp(2rem,6vw,4rem)] leading-[0.95] text-(--ink)">
            An AI that&apos;s actually on your side.
          </h2>
          <p className="mx-auto mt-5 max-w-md text-lg leading-relaxed text-(--smoke)">
        ...
        <div className="flex w-full max-w-md items-start gap-4 text-left">
          <div className="relative size-14 shrink-0 overflow-hidden rounded-full border border-(--line) bg-(--wash)">
            <Image src={activePet.image} alt={activePet.name} fill sizes="56px" className="object-contain p-1" />
          </div>
          <div
            ref={bubbleRef}
            className="relative min-h-18 flex-1 rounded-2xl rounded-tl-sm border border-(--line) bg-(--wash) px-5 py-4"
          >
            <p className="font-mono text-[0.65rem] uppercase tracking-widest" style={{ color: activePet.color }}>
              {activePet.name}
            </p>
            <p className="mt-1 text-base leading-snug text-(--ash)">{activePet.quote}</p>
          </div>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {TRAITS.map((t) => (
            <div key={t.label} className="flex flex-col items-center gap-2 px-2">
              <t.icon className="size-5 text-brand" strokeWidth={1.5} />
              <span className="max-w-[18ch] text-sm text-(--smoke)">{t.label}</span>
            </div>
          ))}
        </div>
      </div>
